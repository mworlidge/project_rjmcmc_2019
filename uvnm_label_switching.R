...
mu <- c(5,10)
sigma2 <- c(1,1)
Z <- do.call(cbind, lapply(1:2, function(i)
  w[i]*dnorm(samples, mu[i], sqrt(sigma2[i]))))
Z <- apply(Z, 1, function(x) which(x==max(x))[1])

result <- uvnm.rjmcmc(y=samples, nsweep=100000, kmax=30, k=2, w=w,
...
plot(mu1[1:length(mu1)], type='l',ylim=c(3,11), ylab="mu1")
plot(mu2[1:length(mu2)], type='l',ylim=c(3,11), ylab="mu2")


plot(density(mu1), main="", xlab="mu1", xlim=c(3,12), lwd=2)
plot(density(mu2), main="", xlab="mu2", xlim=c(3,12), lwd=2)
